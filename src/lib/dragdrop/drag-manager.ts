...
function _createDraggingDiv(doc: Document) {
  ...
  const tabGroup = (_data && 'tabGroup' in _data ? _data['tabGroup'] : undefined) as
    | string
    | undefined;
...
function createDraggingElement(
  ...
  if (draggingHtml) {
    ...
    if (
      window.getComputedStyle(_draggingDiv.firstElementChild).backgroundColor === 'rgba(0, 0, 0, 0)'
    ) {
      (_draggingDiv.firstElementChild as HTMLElement).style.backgroundColor = window
